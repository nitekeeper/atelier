...
| 1.3.2   | 2026-06-12 | Loom chat transport upgraded from default-when-available to **MANDATORY when Loom is available** (`ATELIER_LOOM_COMMS=0` is the ONLY opt-out — `scripts/loom_comms.py::detect`). Added the deregister-on-completion / rejoin-on-demand lifecycle (worker self-`deregister` at terminal closure; `teardown()` end-of-cycle sweep incl. collision-suffix variants; `rejoin()` join-first → stale-session re-register→re-join for returning agents). Loom remains fail-soft; the bridge stays the sole control-plane (TM-006 envelope unchanged). Doc-only change — no `schema_version` bump (the bridge DB schema is unchanged). |
...
## Loom chat transport (MANDATORY when available — bridge stays the control-plane)

When the **loom-agent-chat** plugin is the active chat transport (your briefing's CHANNELS block renders the "Loom team-chat" subsection), its use is **MANDATORY when Loom is available** — not a default you may decline. PEER chat and the plan-phase kickoff meeting MUST ride **Loom**, addressed through the cycle's Loom channel; use it for all such conversational comms rather than the bridge. The ONLY opt-out is the operator env var `ATELIER_LOOM_COMMS=0` (`"0"` is the only disabling value — gated inside `scripts/loom_comms.py::detect`, the single availability choke point); when set, the cycle degrades byte-identical to bridge-only and the obligation is lifted. Availability is decided at the dispatch choke point, not by you: the Loom subsection is injected into your briefing in BOTH modes — team mode and subagent mode — and its presence means Loom was detected and the mandate applies. TM-003 addressivity still applies on Loom: name exactly one recipient per directed send — no broadcast, except the one deliberate `@here` kickoff your team-lead posts. Loom chat bodies are capped at **≤500 chars**; for longer content, write it to `.loom/temp/<name>.md` and post a SHORT pointer message with that path + a 1–2 sentence summary — never paste the full body into chat.

**Deregister on completion / rejoin on demand (the participation lifecycle).** When your job completes (your task reaches terminal closure or you are told to stand down), you MUST **deregister** from the channel as your final Loom action — you are marked gone but the chat history is retained; agents do not stay registered indefinitely. Your self-deregister is the primary mechanism; the orchestrator's end-of-cycle `teardown()` sweep (which also sweeps collision-suffixed name variants like `<name>-2`) is the guaranteed backstop, not a substitute. If you are re-engaged after deregistering (a follow-up wave, a clarification), the orchestrator brings you back via `rejoin()` — join-first; if the join fails on a stale session (non-zero exit), it re-registers you and re-joins. `rejoin()` reports the `assigned_name` you actually re-joined as (a collision rename means you rejoin AS the renamed identity); the orchestrator uses that returned name for subsequent directed sends when it differs from your role-id. A deregister → rejoin → deregister sequence is idempotent and fail-soft.

The terminal `task_result` reply envelope (TM-006) and your heartbeats **ALWAYS** ride the **bridge** control-plane (`scripts/bridge_send.py` → `bridge_messages`), never Loom — Loom carries chat + the kickoff broadcast only (the structured plan-phase meeting thread, `scripts/team_meeting.py`, stays on the bridge). Loom never replaces the mandatory completion reply, and Loom failures are fail-soft: a Loom error never blocks or aborts your task — note it and continue over the bridge. When Loom is unavailable, you communicate normally via the bridge; nothing about the reply contract, the bridge wiring, or any other TM rule changes. Loom is additive, never a substitute for the bridge control-plane.
