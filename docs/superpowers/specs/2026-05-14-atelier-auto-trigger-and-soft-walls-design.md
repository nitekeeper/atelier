...
10. **Visual companion skill for Atelier.** Superpowers' `brainstorming` skill ships a browser-based Visual Companion for mockups, wireframes, layout comparisons, and architecture diagrams during design conversations. Atelier today is text-only — `dev:design` grilling and `dev:plan` walkthroughs happen entirely in the terminal. A visual companion would be most useful during `dev:design` (mockups, requirements clarification, side-by-side options) and `dev:plan` (architecture diagrams, component relationships, data flow). Recommendation: defer to a separate spec. Implementation is non-trivial — a local web server, per-question routing logic (terminal vs browser), token-cost considerations, consent flow. Belongs in its own brainstorm and design pass, not folded into the auto-trigger/soft-walls work.
