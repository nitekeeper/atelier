...
## Loom chat transport (gated — DEFAULT when available, bridge stays the control-plane)

When the **loom-agent-chat** plugin is the active chat transport (your briefing's CHANNELS block renders the "Loom team-chat" subsection), it is the **default** for your conversational comms — PEER chat and the plan-phase kickoff meeting ride **Loom**, addressed through the cycle's Loom channel; use it for all such chat rather than the bridge. TM-003 addressivity still applies on Loom: name exactly one recipient per directed send — no broadcast, except the one deliberate `@here` kickoff your team-lead posts. Loom chat bodies are capped at **≤500 chars**; for longer content, write it to `.loom/temp/<name>.md` and post a SHORT pointer message with that path + a 1–2 sentence summary — never paste the full body into chat. **Once you have fulfilled your purpose** (your task reaches terminal closure or you are told to stand down), **deregister** from the channel as your final Loom action — you are marked gone but the chat history is retained; agents do not stay registered indefinitely.

The terminal `task_result` reply envelope (TM-006) and your heartbeats **ALWAYS** ride the **bridge** control-plane (`scripts/bridge_send.py` → `bridge_messages`), never Loom — Loom carries chat + the kickoff broadcast only (the structured plan-phase meeting thread, `scripts/team_meeting.py`, stays on the bridge). When Loom is unavailable, you communicate normally via the bridge; nothing about the reply contract, the bridge wiring, or any other TM rule changes. Loom is additive, never a substitute for the bridge control-plane.
