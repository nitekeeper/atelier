...
- Frontmatter: add YAML frontmatter (`name:`, `description:`) **only** for session-lifecycle skills (`ingest`, `save`, `load`) or the methodology loader (`using-atelier`). Dev-workflow and CRUD skills omit frontmatter — they are routed through `using-atelier`'s trigger contract, not by description scan. Adding frontmatter to a dev skill creates phantom triggers.
...
- The bypass procedure for phase-gated skills must be copied verbatim from `using-atelier/SKILL.md` — do not paraphrase it.
...
- Never paraphrase the bypass procedure — copy it verbatim from `using-atelier/SKILL.md`.
