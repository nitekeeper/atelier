...
_MINIMAL_DIFF_RULE = (
    ...
    "COVER EVERY REQUIREMENT (compound tasks): minimality is about the SIZE of "
    "the change, never its SCOPE. When the task or its acceptance criteria list "
    "more than one requirement, satisfy EVERY one — 'smallest change' means "
    "fewer lines per requirement, NOT fewer requirements. Dropping or only "
    "partially implementing a stated requirement is not the lazy choice, it is "
    "an INCOMPLETE one. Before you return, re-read the acceptance criteria and "
    "confirm each is covered. "
