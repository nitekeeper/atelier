...
class TestRepoDir:
    def test_resolves_main_repo_when_invoked_from_worktree(
        self, tmp_path, bare_remote, source_repo
    ):
        ...
        result = subprocess.run(
            [sys.executable, str(Path.cwd() / "scripts" / "self_improve.py"), "pull"],
            capture_output=True,
            text=True,
            encoding="utf-8",
            cwd=str(wt_path),
...
class TestCLI:
    ...
    def test_run_tests_pass_exits_0(self, tmp_path, bare_remote, source_repo):
        ...
        result = subprocess.run(
            [
                sys.executable,
                str(Path.cwd() / "scripts" / "self_improve.py"),
                "run-tests",
                str(dest),
            ],
            capture_output=True,
    ...
    def test_cleanup_exits_0(self, tmp_path):
        ...
        exp.mkdir()
        result = subprocess.run(
            [sys.executable, str(Path.cwd() / "scripts" / "self_improve.py"), "cleanup", str(exp)],
            capture_output=True,
            text=True,
            env={**__import__("os").environ, "PYTHONPATH": str(Path.cwd())},
        )
