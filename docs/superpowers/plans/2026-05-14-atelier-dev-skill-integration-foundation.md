# Atelier Dev-Skill Integration — Plan 1: Foundation

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Lay the DB and script foundation for dev-skill integration — migrations for sessions, phases, tasks, and roles; full rewrite of session.py and workflow.py to be DB-backed.

**Architecture:** Three new migrations extend the Atelier DB schema. `session.py` is completely rewritten to read/write a `sessions` table instead of `.ai/work.md`. `workflow.py` replaces hardcoded transition dicts with DB queries against `phases` and `phase_transitions`. A standalone `seed_roles.py` script inserts all 46 world-class expert role profiles.

**Tech Stack:** Python 3.11+, SQLite (via `scripts/db.py`), pytest, existing `scripts/migrate.py` runner.

**Spec:** `docs/superpowers/specs/2026-05-13-atelier-dev-skill-integration.md`

**Plan 2** (skills, hook, cleanup) depends on this plan being complete and all tests passing.
...
| Action | Path | Purpose |
|---|---|---|
| Create | `migrations/002_sessions.sql` | `sessions` table — PM working memory |
| Create | `migrations/003_phases.sql` | `phases`, `phase_transitions`, `skill_gates` tables; 19-phase seed; project phase rename |
| Create | `migrations/004_tasks_parallel.sql` | Add `parallel_group TEXT`; migrate `priority` INTEGER → TEXT |
| Create | `scripts/seed_roles.py` | Idempotent seed for 46 roles + 46 default agents |
| Create | `tests/test_seed_roles.py` | Tests for seed script |
| Rewrite | `scripts/session.py` | DB-backed session management (replaces flat-file) |
| Rewrite | `tests/test_session.py` | Tests for new DB-backed session.py |
| Rewrite | `scripts/workflow.py` | DB-backed phase transitions (replaces hardcoded dicts) |
| Rewrite | `tests/test_workflow.py` | Tests for new DB-backed workflow.py |

---

## Task 1: Migration 002 — Sessions Table
...
python scripts/migrate.py .ai/atelier.db
```

Expected output: `Migrations applied to .ai/atelier.db`

Then verify table exists:
```bash
python -c "
from scripts.db import get_connection
conn = get_connection('.ai/atelier.db')
rows = conn.execute(\"SELECT name FROM sqlite_master WHERE type='table' AND name='sessions'\").fetchall()
print(rows)
"
```
Expected: `[('sessions',)]`
...
```bash
git add migrations/002_sessions.sql
git commit -m "feat: migration 002 — add sessions table"
```

---

## Task 2: Migration 003 — Phase State Machine

**Files:**
- Create: `migrations/003_phases.sql`
...
print('phases:', count)
gates = conn.execute('SELECT * FROM skill_gates').fetchall()
print('skill_gates:', gates)
"
```
Expected: `phases: 19`, 8 rows in skill_gates.

- [ ] **Step 3: Commit**

```bash
git add migrations/003_phases.sql
git commit -m "feat: migration 003 — phase state machine, 19 phases seeded"
```

---

## Task 3: Migration 004 — Tasks Schema Update

**Files:**
- Create: `migrations/004_tasks_parallel.sql`
...
"
```
Expected: `parallel_group` column present; `priority` type is `TEXT`.
...
```
Expected: all pass. If any test asserts `priority == 0` (integer), update it to `priority == 'low'`.

- [ ] **Step 4: Commit**

```bash
git add migrations/004_tasks_parallel.sql
git commit -m "feat: migration 004 — tasks parallel_group, priority INTEGER to TEXT"
```

---

## Task 4: Role Seed Script

**Files:**
- Create: `scripts/seed_roles.py`
- Create: `tests/test_seed_roles.py`

- [ ] **Step 1: Write `scripts/seed_roles.py`**
...
# ---------------------------------------------------------------------------
# Role + default agent definitions
# Each entry:
#   role_name     : str  — inserted into roles.name
#   role_desc     : str  — inserted into roles.description (one-liner)
#   agent_id      : str  — inserted into agents.id (kebab-case, unique)
#   agent_name    : str  — inserted into agents.name (Dr./Prof. name)
#   agent_profile : str  — full profile (expertise, responsibilities, etc.)
# ---------------------------------------------------------------------------
...
- [ ] **Step 2: Write `tests/test_seed_roles.py`**
...
```bash
pytest tests/test_seed_roles.py -v
```
Expected: 7 tests, all PASS.

- [ ] **Step 4: Run seed against the real DB**

```bash
python scripts/seed_roles.py .ai/atelier.db
```
Expected: `Seeded: 46 role(s), 46 agent(s) added to .ai/atelier.db`

- [ ] **Step 5: Commit**

```bash
git add scripts/seed_roles.py tests/test_seed_roles.py
git commit -m "feat: seed 46 world-class expert roles and default agents"
```

---

## Task 5: session.py Rewrite

**Files:**
- Rewrite: `scripts/session.py`
...
```
Expected: all FAIL (ImportError or AttributeError — functions don't exist yet).

- [ ] **Step 3: Rewrite `scripts/session.py`**
...
git commit -m "feat: rewrite session.py — DB-backed PM working memory"
```

---

## Task 6: workflow.py Refactor
...
```
Expected: most FAIL (old VALID_TRANSITIONS / PHASE_GATES do not exist; `is_allow_from_any` undefined; default phase is `design:in-progress` not `design:open`).

- [ ] **Step 3: Rewrite `scripts/workflow.py`**
...
pytest -v
```
Expected: all tests pass (108+ tests from existing suite + new tests).

- [ ] **Step 6: Commit**

```bash
git add scripts/workflow.py tests/test_workflow.py
git commit -m "feat: refactor workflow.py — DB-backed phase transitions and skill gates"
```

---

## Self-Review Checklist (run before handing off to Plan 2)

- [ ] `pytest -v` — all tests green
- [ ] `python scripts/migrate.py .ai/atelier.db` — idempotent, no errors
- [ ] `python scripts/seed_roles.py .ai/atelier.db` — reports 0 new after second run (idempotent)
- [ ] `python scripts/workflow.py check-gate 1 dev:design` — Gate passed (no gate configured)
- [ ] `python scripts/session.py read-latest 1` — returns session or "No session found"
