...
def test_log_bypass_writes_row(project):
    ...
    bypass_id = workflow.log_bypass(
        db_path, pid,
        from_phase="design:open", to_phase="plan:open",
...
def test_log_bypass_records_distinct_audit_events(project):
    ...
    first = workflow.log_bypass(
        db_path, pid,
        from_phase="design:open", to_phase="plan:open",
        reason="first attempt", agent_id="test-agent",
    )
    second = workflow.log_bypass(
        db_path, pid,
        from_phase="design:open", to_phase="plan:open",
        reason="second attempt", agent_id="test-agent",
    )
    assert first != second
    with closing(sqlite3.connect(db_path)) as conn:
        count = conn.execute(
...
def test_log_bypass_cli_writes_row(project, monkeypatch):
    ...
    result = subprocess.run(
        [sys.executable, str(REPO_ROOT / "scripts" / "workflow.py"),
         db_path, "log-bypass", str(pid),
         "design:open", "plan:open",
         "--reason", "from CLI", "--agent", "test-agent"],
        capture_output=True, text=True, encoding="utf-8",
        env={**os.environ, "PYTHONPATH": str(REPO_ROOT),
             "PYTHONUTF8": "1", "HOME": fake_home},
