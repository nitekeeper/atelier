...
- `internal/meeting/SKILL.md` (`read`) — Get meeting details and participants
- `internal/meeting/SKILL.md` (`update`) — Update meeting summary or decisions
- `internal/meeting/SKILL.md` (`delete`) — Remove meeting record and file
- `internal/meeting/SKILL.md` (`list`) — List meetings by date or participant
- `internal/meeting/SKILL.md` (`search`) — Search meetings by title, summary, or decisions

## Procedure

### meeting:create
1. Ask: "Meeting title?" / "Date (YYYY-MM-DD)?" / "Summary?" / "Decisions made?" / "Your agent ID?" / "Participant agent IDs? (comma-separated)"
2. Run: `python3 atelier/scripts/meetings.py create "<title>" "<date>" "<summary>" "<decisions>" "<created_by>" --meetings-dir .ai/meetings`
3. For each participant: `python3 atelier/scripts/meetings.py add-participant <meeting_id> <agent_id>`
4. Confirm: "Meeting recorded: [title] → .ai/meetings/[filename]"

### meeting:read
1. Ask: "Meeting ID?"
2. Run: `python3 atelier/scripts/meetings.py get <id>`
3. Run: `python3 atelier/scripts/meetings.py participants <id>`
4. Display meeting details and participant list.

### meeting:update
1. Ask: "Meeting ID?" / "What to update (summary / decisions / title / date)?"
2. Run: `python3 atelier/scripts/meetings.py update <id> [--summary "..."] [--decisions "..."] [--title "..."] [--date "..."]`
...
2. Run: `python3 atelier/scripts/meetings.py delete <id> --meetings-dir .ai/meetings`
3. Confirm: "Meeting deleted."

### meeting:list
1. Run: `python3 atelier/scripts/meetings.py list`
2. Display results as a table: id | title | date | filename

### meeting:search
1. Ask: "Search query?"
2. Run: `python3 atelier/scripts/meetings.py search "<query>"`
