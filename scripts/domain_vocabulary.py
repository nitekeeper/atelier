...
- ``SUBDOMAINS``     — dict ``{domain: [stable subdomains]}``;
                       Atelier-internal, SOFT-validated (unknown values
                       are accepted; this dict is documentation, not a gate).
- ``TYPE_TO_DOMAIN`` — dict ``{v1_type_string: (domain, subdomain)}``;
...
# ── DOMAINS (spec §6.4, lines 369-379) ──────────────────────────────────
#
...
# ── SUBDOMAINS (spec §6.4, lines 385-394) ───────────────────────────────
#
...
# Callers may pass any string; the lists below document the canonical
...
def assert_valid(domain: str) -> None:
    ...
    Raises :class:`ValueError` with a precise diagnostic listing every
    valid domain in sorted order, so debugging is fast.

    Subdomains are **not** validated here — see :data:`SUBDOMAINS` for
    the soft canonical set; callers may pass any string. Spec §6.4
    documents the rationale ("subdomain enforcement is soft").
    ...
    if domain not in DOMAINS:
        raise ValueError(
            ...
            f"{sorted(DOMAINS)}. Adding a domain requires a spec "
            f"amendment (see internal/memex/domain-vocabulary.md)."
        )
