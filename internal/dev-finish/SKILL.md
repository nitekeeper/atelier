...
4. **Hand off the feature branch — Atelier does NOT push, open a PR, or merge (A6).**
   ...
   > Memex finishes create no team, so SKIP this block and use `(a)`/`(c)` as
   > written.
   >
   ...
   >    the human opens the PR later. Fire it BEFORE step 6 so the durable
   >    artifact survives even if teardown slips.
   >
   > Then continue to step 5 (session record — `--notes "handoff: <branch>"`)
   > and fall through to the EXISTING step 6 (#90 team-teardown record)
   > UNCHANGED. Do NOT also run `(a)` below.
