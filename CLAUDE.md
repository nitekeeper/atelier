...
Atelier now **AUTO-SELECTS the model tier per task** — it no longer defaults every spawn to the most-expensive Opus. The policy lives in `scripts/model_tier.py` (`recommend()`): per teammate/subagent spawn it picks a model TIER by DIFFICULTY from the dev-arc **phase** + the assigned **role** + an optional per-task **difficulty** signal, emitting one of the version-agnostic tier ALIASES `haiku` | `sonnet` | `opus`. The chosen tier flows through the host dispatch path: `scripts/cli_dispatch.py` derives the per-task `--model` flag (via `_host_model_for` → `atelier_entrypoint._default_model_for`) for each `claude -p` worker the deterministic host (`scripts/host_scheduler.py`) spawns. The recommendation is **advisory** — atelier does not refuse to run on other models, and the tables below are TUNABLE in `scripts/model_tier.py`.
